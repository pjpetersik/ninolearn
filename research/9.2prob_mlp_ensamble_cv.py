...
while i<segments:
    ...
    print_header(f"Train Ensemble Nr. {i}")

    start_ind = i * segment_len
    end_ind = (i+1) * segment_len
...
pred_ens = np.zeros((len(testy),2, segments))
predtrain_ens = np.zeros((len(trainy),2, segments))
predfuture_ens = np.zeros((lead_time,2, segments))
